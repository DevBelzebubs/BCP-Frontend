...
@Component({
  selector: 'app-empleado-deposit',
  imports: [CommonModule, FormsModule, RouterModule, BcpInput, BcpButton, BcpAlert],
  templateUrl: './empleado-deposit.html',
  styleUrl: './empleado-deposit.css',
})
export class EmpleadoDeposit {
  ...
  onSubmit() {
    ...
    this.error.set('');
    this.service.depositar({ clienteDni: this.clienteDni, numeroCuenta: this.numeroCuenta, monto: montoNum }).subscribe({
      next: () => { this.loading.set(false); this.success.set(true); },
      error: (err) => { this.loading.set(false); this.error.set(err.error?.message || 'Error al realizar depósito'); },
    });
  }
